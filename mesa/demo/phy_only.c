...
static uint32_t phy_only_port_cnt(void); // ports covered by the '-P' slots

void phy_only_enter(void)
{
    phy_only_mode = 1;
    printf("No switch UIO device found: PHY-only mode (MEPA slots over SPI only)\n");
    printf("PHY-only board conf: port_cnt=%u (from the '-P' slots)\n", phy_only_port_cnt());
}
...
{
    if (strcmp(tag, "port_cnt") == 0) {
        size_t len = snprintf(buf, bufsize, "%u", phy_only_port_cnt());
        if (buflen) {
            *buflen = len;
        }
        return MESA_RC_OK;
...
static uint32_t phy_only_port_cnt(void)
{
    if (phy_only_slot_cnt == 0) {
        return 4; // default
    }
    return phy_only_slot[phy_only_slot_cnt - 1].base + phy_only_slot[phy_only_slot_cnt - 1].ports;
}
                        ...
                        uint8_t mmd, uint16_t reg_num, uint32_t *const data)
{
    ...
    /* phy_only_xfer handles the LAN80xx delayed-read protocol internally
     * (prime + capture on the same fd).  A single xfer call returns the
